using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace E00_Model
{
    public static class cls_LichLamViec
    {
        public static string tb_TenBang = "LT_LICHLAMVIEC";
        public static string col_ID = "ID";
        public static string col_MA = "MA";
        public static string col_MABS = "MABS";
        public static string col_MAKP = "MAKP";
        public static string col_CAP = "CAP";
        public static string col_ID_CHUKY = "ID_CHUKY";
        public static string col_TU = "TU";
        public static string col_DEN = "DEN";
        public static string col_TRANGTHAI = "TRANGTHAI";
        public static string col_NGAYTAO = "NGAYTAO";
        public static string col_NGAYUD = "NGAYUD";
        public static string col_USERID = "USERID";
        public static string col_USERUD = "USERUD";
        public static string col_MACHINEID = "MACHINEID";
        public static string col_ID_LichLamViec = "ID_LICHLAMVIEC";
        
    }
}
